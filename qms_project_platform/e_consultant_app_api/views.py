...
from .serializers import DeleteEconsultantFileSerializer, EconsultantPasswordResetSerializer, EconsultantSessionSerializer, FirebaseEconsultantAuthenticationSerializer, SecureEconsultantForDeletingAssetsSerializer, SecureEconsultantForDownloadingAssetsSerializer, SecureEconsultantForListingFirestoreFilesSerializer, SecureEconsultantForPhoneSerializer, SecureEconsultantForReadingMessagesSerializer, SecureEconsultantForStorageSerializer, SecureEconsultantForSubscriptionClaimsSerializer, SecureEconsultantForTelemedicsSerializer, SecureEconsultantToUploadToStorageSerializer  
...
@extend_schema(responses=FirebaseEconsultantAuthenticationSerializer)
@api_view(['POST'])
@csrf_exempt
def authenticate_econsultant(request):
    logger.info("Received authentication request payload")

    serializer = FirebaseEconsultantAuthenticationSerializer(data=request.data)
...
@extend_schema(responses=EconsultantSessionSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_messaging(request):
    logger.info("Received request to secure user for messaging")
    
    serializer = EconsultantSessionSerializer(data=request.data)
    if not serializer.is_valid():
        logger.error("Invalid data received for authentication")
        return JsonResponse(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema(responses=SecureEconsultantForStorageSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_storage(request):
...
@extend_schema(responses=SecureEconsultantForTelemedicsSerializer)
@api_view(['POST'])
def secure_econsultant_for_telemedics(request):
    
    serializer = SecureEconsultantForTelemedicsSerializer(data=request.data)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema(responses=SecureEconsultantForReadingMessagesSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_reading_messages(request):
    logger.info("Received request to secure users for messages")
    serializer = SecureEconsultantForReadingMessagesSerializer(data=request.data)
...
@extend_schema(responses=SecureEconsultantToUploadToStorageSerializer)
@csrf_exempt
@api_view(['POST'])
def secure_econsultant_to_upload_to_storage(request):
    logger.info("Received request to secure users for storage")
    serializer = SecureEconsultantToUploadToStorageSerializer(data=request.data)
    ...
    try:
        ...
        db = firestore.client()
        user_doc_ref = db.collection('econsultant').document(user_id)
        stored_session = user_doc_ref.get().to_dict().get('session_token')
...
@extend_schema(responses=SecureEconsultantForDownloadingAssetsSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_downloading_assets(request):
    logger.info("Received request to secure users for downloading")
    serializer = SecureEconsultantForDownloadingAssetsSerializer(data=request.data)
...
@extend_schema(responses=SecureEconsultantForDeletingAssetsSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_deleting_assets(request):
    logger.info("Received request to secure users for storage")
    
    serializer = SecureEconsultantForDeletingAssetsSerializer(data=request.data)
    if not serializer.is_valid():
        logger.error("Invalid data received for securing users for storage")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema(responses=SecureEconsultantForSubscriptionClaimsSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_subscription_claims(request):
    logger.info("Received request to secure users for downloading")
    serializer = SecureEconsultantForSubscriptionClaimsSerializer(data=request.data)
...
@extend_schema(responses=SecureEconsultantForPhoneSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_phone_number_verification(request):
    logger.info("Received request to secure users for downloading")
    serializer = SecureEconsultantForPhoneSerializer(data=request.data)
...
@extend_schema(responses=EconsultantPasswordResetSerializer)
@api_view(['POST'])
@csrf_exempt
def econsultant_password_reset(request):
    logger.info("Received request to reset user's password")


    serializer = EconsultantPasswordResetSerializer(data=request.data)
    if not serializer.is_valid():
...
@extend_schema(responses=DeleteEconsultantFileSerializer)
@api_view(['POST'])
@csrf_exempt
def econsultant_delete_file_from_firestore(request):
    ...
    serializer = DeleteEconsultantFileSerializer(data=request.data)
    if not serializer.is_valid():
...
@extend_schema(responses=SecureEconsultantForListingFirestoreFilesSerializer)
@api_view(['POST'])
@csrf_exempt
def secure_econsultant_for_listing_firestore_files(request):
    logger.info("Received request to secure users for listing firstore files")
    
    serializer = SecureEconsultantForListingFirestoreFilesSerializer(data=request.data)
    if not serializer.is_valid():
        logger.error("Invalid data received for securing users for storage")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
